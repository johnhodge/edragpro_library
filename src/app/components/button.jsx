...
function GlobalButton(props) {
    ...
    var icon = {
        check: <icons_1.Check theme={props.theme}/>,
        code: <icons_1.Code theme={props.theme}/>,
        login: <icons_1.Login theme={props.theme}/>,
    };
    ...
    if (props.eventType === 'link') {
        return (<link_1.default className={classNames} href={props.href} title={props.title} type={props.type}>
        <span>{props.children}</span>
        {props.icon ? <span>{icon[props.icon]}</span> : ''}
      </link_1.default>);
    }
    else if (props.eventType === 'login' || 'logout' || 'other') {
        return (<button className={classNames} onClick={props.eventType === 'login'
                ? function () { return (0, react_2.signIn)(props.id); }
                : props.eventType === 'logout'
                    ? function () { return (0, react_2.signOut)(); }
